<script setup>

</script>

<template>
<div class="content-right">
    <div class="sidebar-creator card">
        <div class="creator-hd">
            <i class="iconfont icon-jurassic_edit-user"></i>
            <span class="creator-hd-title">创作中心</span>
            <div class="creator-hd-drafts">草稿箱(2)</div>
        </div>
        <div class="creator-bd">
            <ul>
                <li>
                    <i class="iconfont icon-shiyongwendang"></i>
                    <p>回答问题</p>
                </li>
                <li>
                    <i class="iconfont icon-shiyongwendang"></i>
                    <p>回答问题</p>
                </li>
                <li>
                    <i class="iconfont icon-shiyongwendang"></i>
                    <p>回答问题</p>
                </li>
                <li>
                    <i class="iconfont icon-shiyongwendang"></i>
                    <p>回答问题</p>
                </li>
            </ul>
        </div>
        <div class="creator-fd">
            <div class="creator-fd-list">
                <ul>
                    <li>
                        1
                    </li>
                </ul>
                <ul>
                    <li>2</li>
                </ul>
                <ul>
                    <li>3</li>
                </ul>
                <ul>
                    <li>3</li>
                </ul>
            </div>
        </div>
    </div>
    <!-- 第二个 推荐关注 -->
    <div class="sidebar-recommend card">
        <div class="recommend-hd">
            <i class="iconfont icon-jurassic_edit-user"></i>
            <span class="creator-hd-title">推荐关注</span>
        </div>
        <div class="recommend-bd">
            <ul>
                <li>
                    <img src="@/assets/image/pic7.jpg" alt="">
                    <div class="recommend-content">
                        <div class="recommend-head">房澳</div>
                        <div class="recommend-detail">「游戏」领域答主</div>
                    </div>
                    <div class="recommend-button">
                        <i class="iconfont icon-jiahao"></i>关注
                    </div>
                </li>
                 <li>
                    <img src="@/assets/image/pic7.jpg" alt="">
                    <div class="recommend-content">
                        <div class="recommend-head">房澳</div>
                        <div class="recommend-detail">互联网」领域答主</div>
                    </div>
                    <div class="recommend-button">
                        <i class="iconfont icon-jiahao"></i>关注
                    </div>
                </li>
                 <li>
                    <img src="@/assets/image/pic7.jpg" alt="">
                    <div class="recommend-content">
                        <div class="recommend-head">房澳</div>
                        <div class="recommend-detail">「数码」领域答主</div>
                    </div>
                    <div class="recommend-button">
                        <i class="iconfont icon-jiahao"></i>关注
                    </div>
                </li>
            </ul>
        </div>
    </div>

    <div class="sidebar-category card">
        <ul>
            <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shu"></use></svg>
                <p>sss</p>
            </li>
             <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shu"></use></svg>
                <p>sss</p>
            </li>
             <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shu"></use></svg>
                <p>sss</p>
            </li>
             <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shu"></use></svg>
                <p>sss</p>
            </li>
             <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shandian1"></use></svg>
                <p>sss</p>
            </li>
             <li>
                <svg class="icon-font">
                <use xlink:href="#icon-shu"></use></svg>
                <p>sss</p>
            </li>
            
        </ul>
    </div>

    <div class="sidebar-namlist card">
        4
    </div>
</div>
</template>

<style scoped>

</style>
